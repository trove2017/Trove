...
public class MultiColourRenderer implements Renderer {
	private stringConstant strConstant=new stringConstant();
    public final String COLUMN_COLOUR_LIST = strConstant.getGephiHallmarkColorList();
    ...
    public PreviewProperty[] getProperties() {
        return new PreviewProperty[]{
            ...
            "Multi Colour Nodes", "Make a node be coloured more then one hue",
    ...
    private void renderProcessing(Item item, ProcessingTarget target, PreviewProperties properties) 
    {
        ...
        float alpha = n.getNodeData().alpha();
        ...
        if (colourArray.size() > 0) 
        {
            ...
            for (int i = 0; i < colourArray.size(); i++) 
            {
            	...
            	Color color=colourArray.get(i);
            	int r=color.getRed();
            	int g=color.getGreen();
            	int b=color.getBlue();
            	Color c2=new Color((float)r/255, (float)g/255, (float)b/255, alpha);
            	g2.setColor(c2);
