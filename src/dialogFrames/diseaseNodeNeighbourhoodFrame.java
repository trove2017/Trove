...
public class diseaseNodeNeighbourhoodFrame extends JDialog{
	private JPanel settingPanel, buttonPanel, neighbourPanel, neighbourhoodGraphPanel;
	private JButton cancelButton;
	private JList<String> diseaseNodeList;
	private JTextArea neighbourTextArea;
	private JScrollPane diseaseNodeScrollPane, neighbourScrollPane;
	...
	private String selectedDiseaseNode;
	private String neighbour;
	private ArrayList<String> neighbourList;
	
	public diseaseNodeNeighbourhoodFrame(ArrayList<String> selectedDiseaseNode, Gephi_graph graph)
	{
		//initialize constants
		listOfDiseaseNode=selectedDiseaseNode;
	...
	private void visualizeGraph(JPanel nGraphPanel)
	{
		...
		*/
        ...
        {
        	if(listOfDiseaseNode.contains(n.getNodeData().getLabel())==false)
        		val.add(Double.valueOf(1));
        }
        int i=0;
        for(Node n: nGraph.getNodes())
	...
	private void initializeComponents()
	{
		...
		diseaseNodeList.addListSelectionListener(new ListSelectionListener() 
		{
		...
		diseaseNodeScrollPane.setPreferredSize(new Dimension(150, 380));
	...
	private void visualizeSelectedDiseaseNodeInGraph(String node)
	{
		//Preview configuration
		nGraph.writeLock();
		...
		for(Node n: nGraph.getNodes())
		{
			if(node.compareTo(n.getNodeData().getLabel())==0) //selected disease node (red, opaque)
			{
			...
			else //other nodes
			{
				...
				if(e1==null && e2==null) //n is not a neighbour to the selected disease node
				{
					if(listOfDiseaseNode.contains(n.getNodeData().getLabel())==true)//non-selected disease node (cyan, transparent)
	...
	private void diseaseNodeList_valueChanged(ListSelectionEvent e) 
	{
		...
		{
			if(diseaseNodeList.getSelectedValue()!=null)
			{
				selectedDiseaseNode = diseaseNodeList.getSelectedValue().toString();
				System.out.println(selectedDiseaseNode);
				visualizeSelectedDiseaseNodeInGraph(selectedDiseaseNode);
			}
		}
	}
